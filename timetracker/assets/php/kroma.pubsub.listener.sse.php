<?php
  ...
  $REQ_URL  = isset($_REQUEST['url'])   ? $_REQUEST['url']    : false;
  ...
  function onMessage($redis, $chan=REDIS_DEBUG_CHANNEL, $msg, $event="data") {
    ...
    // if we are looking at a specific url
    if($REQ_URL) {
  ...
  function sendReportSummary($url="", $from=0, $to=0) {
  ...
  function sendReport($url="", $from=0, $to=false) {
    global $db, $reply;
  ...
  if(DEBUG) {
    sendEvent("debug", json_encode($debug));
  }
